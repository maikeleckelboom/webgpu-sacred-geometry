...
export async function startArchitectureRenderer(canvas: HTMLCanvasElement): Promise<ArchitectureRenderer> {
  const forceFallback = new URLSearchParams(window.location.search).get('webgpu') === 'off'

  if (forceFallback) {
    throw new Error('WebGPU rendering is disabled for this view. The page content remains available without the architectural network graphic.')
  }

  if (!navigator.gpu) {
    throw new Error('WebGPU is not available in this browser. The architectural network graphic needs a WebGPU-capable browser; the page content remains available.')
  }
  ...
  function renderFrame(time: number): void {
    ...
    uniforms.set([canvas.width, canvas.height, Math.min(window.devicePixelRatio || 1, 2.4), aspect], 20)
...
function addSheet(nodes: NodePoint[], lines: LineSegment[], planes: PlaneVertex[], spec: SheetSpec): SheetData {
  const points: Vec3[][] = []
  ...
  for (let row = 0; row < spec.rows; row += 1) {
    ...
    for (let column = 0; column < spec.columns; column += 1) {
      ...
      const edge = row === 0 || column === 0 || row === spec.rows - 1 || column === spec.columns - 1
  ...
  addQuad(planes, topLeft, topRight, bottomRight, bottomLeft, spec.planeAlpha, spec.tone + 0.08, spec.phase, spec.focus)
  ...
  if (spec.diagonals !== 'none') {
    for (let row = 0; row < spec.rows - 1; row += 1) {
      for (let column = 0; column < spec.columns - 1; column += 1) {
        ...
        if (drawForward) {
          addSegment(lines, points[row][column], points[row + 1][column + 1], 0.46, spec.lineAlpha * 0.48, spec.tone + 0.04, spec.phase + 1.1, spec.layer, spec.focus)
        }

        if (drawBack) {
          addSegment(lines, points[row + 1][column], points[row][column + 1], 0.38, spec.lineAlpha * 0.32, spec.tone + 0.06, spec.phase + 1.6, spec.layer, spec.focus)
        }
      }
    }
  }
...
function addPrimaryTrusses(
  ...
  addPolyline(lines, [p(middleDeck, 1, 0), p(middleDeck, 1, 2), p(foregroundFrame, 2, 2), p(foregroundFrame, 2, 5)], 2.35, 0.62, 0.05, 8.2, 1, 1)
  addPolyline(lines, [p(lowerDeck, 0, 0), p(lowerDeck, 0, 2), p(foregroundFrame, 1, 3), p(foregroundFrame, 1, 5)], 2.05, 0.54, 0.08, 8.8, 0.96, 0.9)
  addPolyline(lines, [p(rearVault, 3, 0), p(middleDeck, 3, 1), p(foregroundFrame, 4, 3), p(foregroundFrame, 4, 5)], 1.35, 0.32, 0.16, 9.4, 0.78, 0.58)
  addPolyline(lines, [p(middleDeck, 0, 3), p(foregroundFrame, 0, 2), p(foregroundFrame, 5, 3), p(lowerDeck, 2, 3)], 0.85, 0.22, 0.18, 10.2, 0.86, 0.64)
  addPolyline(lines, [p(rearVault, 0, 2), p(rearVault, 4, 4), p(foregroundFrame, 3, 4)], 0.7, 0.16, 0.28, 10.8, 0.48, 0.25)

  for (const position of [p(middleDeck, 1, 2), p(foregroundFrame, 2, 2), p(foregroundFrame, 2, 5), p(lowerDeck, 0, 2), p(foregroundFrame, 1, 3)]) {
    addNode(nodes, position, 8.4, 0.86, 0.04, 11.6, 1)
  }
}

function addMeshSurfaces(lines: LineSegment[], middleDeck: SheetData, foregroundFrame: SheetData, lowerDeck: SheetData): void {
  addMeshPatch(
    lines,
    p(middleDeck, 1, 2),
...
function addRailBundles(nodes: NodePoint[], lines: LineSegment[], middleDeck: SheetData, foregroundFrame: SheetData, lowerDeck: SheetData): void {
  addRailBundle(lines, p(middleDeck, 2, 1), p(foregroundFrame, 4, 5), [0.012, -0.055, 0.015], 7, 1.18, 0.42, 0.04, 16.2, 1, 0.95)
  addRailBundle(lines, p(lowerDeck, 0, 0), p(foregroundFrame, 2, 5), [0.006, -0.046, 0.02], 5, 0.98, 0.34, 0.06, 16.9, 0.96, 0.88)
  addRailBundle(lines, p(middleDeck, 0, 4), p(foregroundFrame, 5, 2), [-0.02, 0.038, 0.012], 4, 0.72, 0.22, 0.12, 17.5, 0.84, 0.72)

  for (const point of [p(foregroundFrame, 0, 4), p(foregroundFrame, 2, 5), p(foregroundFrame, 4, 5), p(foregroundFrame, 5, 2)]) {
    addNode(nodes, point, 9.6, 0.96, 0.03, 18.1, 1)
  }
}

function addForegroundTower(nodes: NodePoint[], lines: LineSegment[], planes: PlaneVertex[], foregroundFrame: SheetData): void {
  const tower = [
  ...
  for (const [start, end, width, alpha] of edges) {
    addSegment(lines, nodes[tower[start]].position, nodes[tower[end]].position, width, alpha, 0.025, 22 + start * 0.3 + end * 0.11, 1, 1)
  }
  ...
  addQuad(planes, nodes[tower[0]].position, nodes[tower[2]].position, nodes[tower[4]].position, nodes[tower[5]].position, 0.045, 0.1, 24.2, 0.95)
  addPolyline(lines, [p(foregroundFrame, 2, 3), nodes[tower[0]].position, nodes[tower[4]].position], 1.15, 0.34, 0.04, 24.8, 1, 0.98)
}

function addDepthGhosts(lines: LineSegment[], rearVault: SheetData, middleDeck: SheetData, foregroundFrame: SheetData): void {
  addPolyline(lines, [p(rearVault, 0, 1), p(rearVault, 2, 3), p(middleDeck, 0, 5)], 3.2, 0.055, 0.64, 12.2, 0.28, 0.05)
  addPolyline(lines, [p(rearVault, 4, 0), p(middleDeck, 3, 2), p(foregroundFrame, 0, 4)], 2.8, 0.05, 0.58, 12.8, 0.34, 0.08)
  addPolyline(lines, [p(rearVault, 1, 5), p(middleDeck, 4, 4), p(foregroundFrame, 5, 5)], 2.4, 0.045, 0.54, 13.4, 0.32, 0.05)
}
...
function addMeshPatch(
  ...
  for (let index = 1; index < uCount; index += 1) {
    ...
    addSegment(lines, bilerp3(a, b, c, d, u, 0), bilerp3(a, b, c, d, u, 1), 0.26, alpha, tone, phase + index * 0.13, layer, focus)
  }

  for (let index = 1; index < vCount; index += 1) {
    const v = index / vCount
    addSegment(lines, bilerp3(a, b, c, d, 0, v), bilerp3(a, b, c, d, 1, v), 0.24, alpha * 0.9, tone, phase + index * 0.17, layer, focus)
  }
  ...
  for (let u = 0; u < uCount - 1; u += 2) {
    for (let v = 0; v < vCount - 1; v += 2) {
      ...
      const u1 = (u + 1) / uCount
      const v1 = (v + 1) / vCount
      addSegment(lines, bilerp3(a, b, c, d, u0, v0), bilerp3(a, b, c, d, u1, v1), 0.18, alpha * 0.48, tone + 0.08, phase + u * 0.19 + v * 0.11, layer, focus)
    }
  }
}
...
function addRailBundle(
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    addSegment(lines, add3(start, offset), add3(end, offset), width * (0.72 + weight * 0.42), alpha * (0.58 + weight * 0.42), tone, phase + index * 0.16, layer, focus)
  }
}

function addNode(nodes: NodePoint[], position: Vec3, radius: number, alpha: number, tone: number, phase: number, kind: number): number {
  const index = nodes.length
...
function addPolyline(
  ...
  for (let index = 0; index < points.length - 1; index += 1) {
    addSegment(lines, points[index], points[index + 1], width, alpha, tone, phase + index * 0.37, layer, focus)
  }
}
...
function resizeCanvas(canvas: HTMLCanvasElement, device: GPUDevice): boolean {
  ...
  const width = Math.max(1, Math.min(device.limits.maxTextureDimension2D, Math.floor(rect.width * pixelRatio)))
  const height = Math.max(1, Math.min(device.limits.maxTextureDimension2D, Math.floor(rect.height * pixelRatio)))
