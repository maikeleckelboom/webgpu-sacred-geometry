...
function createMandalaGeometry(): MandalaGeometry {
...
function addInnerLattice(builder: GeometryBuilder): void {
  ...
  for (let ringIndex = 0; ringIndex < rings.length; ringIndex += 1) {
    ...
    const points = builder.polygon(
      6,
      radius,
      rotation,
      0.0016,
      fade(ringIndex % 2 ? amber : cyan, 0.62),
      0.25,
    );
...
function addPrecisionMesh(builder: GeometryBuilder): void {
  ...
  for (let layer = 0; layer < radii.length; layer += 1) {
    ...
    builder.polyline(
      ...
      true,
      0.00085,
      fade(layer % 2 === 0 ? cyan : white, layer < 3 ? 0.48 : 0.36),
    );
  ...
  for (let index = 0; index < 24; index += 1) {
    ...
    builder.line(
      ...
      polar(0.826, angle),
      0.00078,
      fade(color, index % 2 === 0 ? 0.34 : 0.22),
    );
  }
...
function addPanelCircuits(builder: GeometryBuilder): void {
  for (let sector = 0; sector < 6; sector += 1) {
    ...
    const axisA: Point = [
      center[0] + Math.cos(midAngle) * 0.106,
      center[1] + Math.sin(midAngle) * 0.106,
    ];
    const axisB: Point = [
      center[0] - Math.cos(midAngle) * 0.106,
      center[1] - Math.sin(midAngle) * 0.106,
    ...
    const crossA: Point = [
      center[0] + Math.cos(tangent) * 0.074,
      center[1] + Math.sin(tangent) * 0.074,
    ];
    const crossB: Point = [
      center[0] - Math.cos(tangent) * 0.074,
      center[1] - Math.sin(tangent) * 0.074,
    ...
    for (let index = -2; index <= 2; index += 1) {
      ...
      const bead = [
        center[0] + Math.cos(beadAngle) * 0.092,
        center[1] + Math.sin(beadAngle) * 0.092,
      ] as Point;
      builder.sprite(
        bead,
        index === 0 ? 0.007 : 0.0045,
        fade(index === 0 ? amber : white, 0.58),
        1,
      );
    }
  }
