...
export async function startWavesRenderer(canvas: HTMLCanvasElement): Promise<WavesRenderer> {
  if (!navigator.gpu) {
    throw new Error('WebGPU is not available in this browser. The graphite wave field needs a WebGPU-capable browser.')
  }
  ...
  function renderFrame(time: number): void {
    ...
    uniforms.set([canvas.width, canvas.height, pixelRatio, canvas.width / Math.max(1, canvas.height)], 0)
...
function streamlineStyle(point: Point, rowAmount: number, lane: number): SegmentStyle {
  ...
  const mainBand = Math.exp(-square((rowAmount - 0.49) / 0.055)) * ellipticalFocus(point, 0.5, 0.54, 0.44, 0.13)
...
function ellipticalFocus(point: Point, centerX: number, centerY: number, radiusX: number, radiusY: number): number {
...
function resizeCanvas(canvas: HTMLCanvasElement, device: GPUDevice): boolean {
  ...
  const pixelRatio = Math.min(window.devicePixelRatio || 1, 3.25)
